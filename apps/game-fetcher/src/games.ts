...
export const getGames = async (
	gamesUrl: string,
): Promise<Result<GetGamesResponse, Error>> => {
	...
	if (!fetchResponse.ok) {
		return Err(
			new Error(`${fetchResponse.status} - ${fetchResponse.statusText}`),
		);
	}

	const [jsonResponse, jsonParsingError] =
		await encasePromise<FetchGamesResponse>(fetchResponse.json());

	if (jsonParsingError) {
		return Err(jsonParsingError);
	}

	const [validatedResponse, validationError] = await encasePromise(
		FetchGamesResponse.parseAsync(jsonResponse),
	);
