...
export async function corsMiddleware(c: AppContext, next: Next): Promise<void> {
  ...
  const shouldAddCors = origin && await shouldAddCorsHeader(origin, c?.env?.DB);
...
async function shouldAddCorsHeader(origin: string | null, db: KVNamespace | undefined): Promise<boolean> {
  if (!origin || !db) {
    return false;
  }
